...
export default function Collision(engine: Engine,
  ...
  return engine.runIteratorOn([ "position" ], (root: Node, entity: Entity) => {
    return addEntity(root, entity, maskFor(entity).bounds());
...
export class Node extends Immutable.Record({
  ...
  public query(entity: Entity, bounds?: Bounds): Immutable.Map<string, Entity> {
    let hasNPhase1 = hasNPhase(entity);
    let mask1 = maskFor(entity);
    bounds = bounds || mask1.bounds();

    if (this.children) {
      return this.children.reduce((memo: Immutable.Map<string, Entity>,
                                   node: Node) => {
        if (checkBounds(node, bounds)) {
      ...
      return this.entities.reduce((memo: Immutable.Map<string, Entity>,
                                   value: [ Bounds, Entity ]) => {
        ...
        if (entity.getIn([ "meta", "id" ]) !== value[1].getIn([ "meta", "id" ])
            ...
            && (!narrowPhase || checkMasks(mask1, maskFor(value[1])))) {
